...
def evaluate(board,color):
  if str(board) in ttable:
    return ttable[str(board)]
...
def ComputerMove(board):
  ttable={}
  def ordereval(move):
    if board.is_attacked_by('w' not in str(board.fen()),chess.parse_square(str(move)[:2])):
      return 300
    if board.gives_check(move):
      return 400
    if board.is_capture(move):
      return 200
    if board.color_at(chess.parse_square(str(move)[2:]))=='w' not in str(board.fen()):
      return int(board.piece_type_at(chess.parse_square(str(move)[2:])))
    return 0
  ...
  moves=sort(list(board.legal_moves))          
  eval=minimax(board,depth,-100000000000,100000000000,iswhite)
  return eval[1]

def PlayerMove(board):
  move='...'
  while move not in str(board.legal_moves):
    move=input('Choose Move\n')
    if move+',' not in str(board.legal_moves) and move+')' not in str(board.legal_moves):
      move='...'
  return move
